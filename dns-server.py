...
soa_record = SOA(
    ...
    rname=D.mail,  # email of the domain administrator
    times=(
        ...
        for name, rrs in records.iteritems():
            if name+'.' == qn:
                for rdata in rrs:
                    ...
                    if qt in ['*', rqt]:
                        reply.add_answer(RR(rname=qname, rtype=QTYPE.__dict__['reverse'][rqt], rclass=1, ttl=TTL, rdata=rdata))

        if qn.startswith('get.'):
            for subzone in CACHE:
                if subzone == qn[4:]:
                    reply.add_answer( RR( rname=qname, rtype=QTYPE.A, rclass=1, ttl=TTL, rdata=A(IP) ) )
                    CACHE.remove(subzone)
                    break
        else:
            CACHE.add(qn)
    print CACHE
